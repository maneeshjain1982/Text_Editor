...
> Last updated: 2026-09-16 · last commit at handoff: `aace687` on `main`
...
│  ├─ package.json                       exports: ".", "./docx", "./styles.css"; peerDep vue
│  ├─ vite.config.ts                     library build: 2 entries, dependencies externalised, vite-plugin-dts
│  ├─ vitest.config.ts                   happy-dom, css: true (needed for ?raw CSS imports)
│  ├─ README.md                          API reference (props, events, methods, CSS variables)
│  ├─ test/                              Vitest unit tests (fixtures.ts, docx.spec.ts, editor.spec.ts)
│  └─ src/                               (map in §3)
...
└─ examples/                             NOT in the workspace, on purpose (they consume the real .tgz)
   ...
   │  └─ README.md                       how to run it + an 11-step integration walkthrough
...
| `services/content.ts` | `normalizeContent()`: turns `{type:'doc',content:[]}` into a doc with one paragraph (see P1) |
...
| Unit tests (31) | `npm run test -w @local/rich-editor` |
| Playground E2E (13) | `npx playwright test` (Edge; `PW_CHANNEL=chrome` for Chrome) |
...
| Full package validation (8 checks + 8 browser tests) | `cd examples/sample-app-tests && npm run validate` |
...
At handoff, every suite passes: 31 unit, 13 playground E2E, 16 sample-app validation (the browser tests also passed 3 repeated runs).
