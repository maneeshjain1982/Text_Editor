# Package validation: sample app and tests

These two projects check that `@local/rich-editor` works as an **installed package**. Unlike the playground, which reads the editor's source code directly, they install the packed `.tgz` file exactly as your dashboard would.
...
| [`sample-app-tests/`](sample-app-tests) | A separate test project (Playwright). It checks the installed package, then tests the sample app's **production build** in a real browser. |

Both folders are standalone npm projects. They are deliberately **not** part of the root npm workspace, so nothing can be resolved from the editor's source by mistake.
...
2. `npm install` of that tarball into `sample-app`
3. The package checks (Node, no browser)
4. A production build of the sample app, including `vue-tsc`, served with `vite preview` on port 4173
5. The browser tests against that build, in the installed Microsoft Edge (`PW_CHANNEL=chrome` for Chrome)
